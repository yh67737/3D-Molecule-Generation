...
from src.models.Moldiff_network.transition import ContigousTransition, GeneralCategoricalTransition
from src.models.Moldiff_network.graph import NodeEdgeNet

from src.models.Moldiff_network.common import *
from src.models.Moldiff_network.diffusion import *
...
class MolDiff(Module):
    def __init__(self,
        ...
        **kwargs
        ...
        if config.denoiser.backbone == 'NodeEdgeNet':
            self.denoiser = NodeEdgeNet(node_dim, edge_dim, **vars(config.denoiser))
        else:
    ...
    def forward(self, data, t, target_node_mask, target_edge_mask):
        """
        [重构后]
        让 MolDiff 的行为与 E-DiT 兼容。
        输入加噪的 data 对象和掩码，仅对目标原子/边进行预测。
        """
        # 1. 从输入的 PyG data 对象中解包加噪后的数据
        #    这里的输入已经是 train_B.py 加噪后的结果
        pos_pert = data.pos
        h_node_pert = data.x  # one-hot 编码的原子类型
        edge_index = data.edge_index
        h_halfedge_pert = data.edge_attr  # one-hot 编码的边类型
        batch_node = data.batch if hasattr(data, 'batch') and data.batch is not None else torch.zeros(data.num_nodes,
                                                                                                      dtype=torch.long,                                                                                              device=data.x.device)

        # MolDiff 使用 half-edge 逻辑，我们需要创建对应的全边 (undirected) 版本
        edge_index_undirected = torch.cat([edge_index, edge_index.flip(0)], dim=1)
        h_edge_pert = torch.cat([h_halfedge_pert, h_halfedge_pert], dim=0)  # 假设输入已经是 half-edge 的属性
        batch_edge = batch_node[edge_index_undirected[0]]

        # 2. 特征嵌入
        time_embed_node = self.time_emb(t.index_select(0, batch_node))
        h_node_pert_embedded = torch.cat([self.node_embedder(h_node_pert), time_embed_node], dim=-1)

        time_embed_edge = self.time_emb(t.index_select(0, batch_edge))
        h_edge_pert_embedded = torch.cat([self.edge_embedder(h_edge_pert), time_embed_edge], dim=-1)

        # 3. Denoiser (NodeEdgeNet) 对【整个图】进行处理
        h_node_denoised, pos_node_denoised, h_edge_denoised = self.denoiser(
            h_node=h_node_pert_embedded,
            pos_node=pos_pert,
            h_edge=h_edge_pert_embedded,
        ...
        # 4. 应用掩码，只为目标原子/边提取特征并进行解码
        #    这实现了用户要求的 "只输出目标原子"

        # 4.1. 提取目标节点的特征
        target_h_node_denoised = h_node_denoised[target_node_mask]
        target_pos_node_denoised = pos_node_denoised[target_node_mask]

        # 4.2. 提取目标边的特征
        # 注意: denoiser 输出的是全边 (undirected) 特征，我们需要映射回半边 (half-edge)
        n_halfedges = h_halfedge_pert.shape[0]
        # 融合正反向边的特征
        h_halfedge_denoised_fused = h_edge_denoised[:n_halfedges] + h_edge_denoised[n_halfedges:]
        target_h_halfedge_denoised = h_halfedge_denoised_fused[target_edge_mask]

        # 5. 解码器 (Decoders)
        pred_node_logits = self.node_decoder(target_h_node_denoised)
        pred_pos_x0 = target_pos_node_denoised
        pred_halfedge_logits = self.edge_decoder(target_h_halfedge_denoised)

        # 6. 返回一个与 train_B.py 兼容的字典
        return {
            'atom_type_logits': pred_node_logits,
            'predicted_r0': pred_pos_x0,
            'bond_logits': pred_halfedge_logits,
        }
